###
view: fotc_dp_ce_gl_detail_adjustment {
  required_access_grants: [explorer_security_bfa_all]
  label: "Manual Adjustments"
  derived_table: {
    sql: select * from @{server_name_fine}.@{schema_name}.FOTC_DP_CE_GL_DETAIL_ADJUSTMENT_V01_00_HBUK
          WHERE MI_DATA_SOURCE='MI_Manual_Adjustment';;
    #datagroup_trigger: BFA_glsdi_datagroup_current_year
  }
...
##############Format###############
  ...
  dimension: pre_allocation_engine {
    ...
    type: string
    sql: CASE
          WHEN ${TABLE}.pre_allocation_engine = false THEN "No"
          WHEN ${TABLE}.pre_allocation_engine = true THEN "Yes"
          END;;
  }
